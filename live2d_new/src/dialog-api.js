  ...
  if (audioUrl) {
    try {
      ...
      await new Promise((res, rej) => {
        ...
        audio.onerror = (e) => {
            console.warn('[Live2D] Audio load failed for duration check:', audioUrl, e);
            rej(e);
        };
      });
      if (Number.isFinite(audio.duration) && audio.duration > 0) {
        ...
        // Audio load failed, ignore and fallback to text duration
    }
  }
  // 如果音频加载失败，使用文本时长，但至少给 3 秒 (比之前的 1.5 秒长一点，防止太快消失)
  const duration = Math.max(textDuration, audioDuration ?? 0, 3.0);
  return { duration, audioDuration, textDuration };
}

export async function initLive2dWithDialogue(options = {}) {
  // ✅ 透传所有options(包括enablePanel)到initLive2d
  ...
  async function say({
    ...
    if (!res?.ok && character.model?.motion) {
      try {
        // 强制使用 Idle 组的第一个动作，或者直接使用空动作来触发音频
        // 注意: 如果 motion 找不到，音频可能不会播放。
        // 我们尝试使用 "Idle" 组
        character.model.motion("Idle", 0, 2, extra.sound ? { sound: extra.sound, crossOrigin: "anonymous", volume: extra.volume } : undefined);
        res = { ok: true, spec: { kind: "motion", group: "Idle", indexInGroup: 0 } };
      } catch (_) {}
    }
